...
class ViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        if LMIDI.sourceNames.isEmpty || LMIDI.destinationNames.isEmpty {                            //MIDIデバイスが存在しない
            ...
            midiDevice = LMIDI(sourceName: LMIDI.sourceNames.first!,                                //MIDIデバイスのインスタンスを生成する
...
extension ViewController {
    ...
    @IBAction func noteButton(_ sender: Any) {
        midiDevice?.noteOn(ch: 0, noteNumber: LMIDI.noteNumber(noteName: noteButton.title)!, velocity: 64)      //ノートオンを送信する
        
        Timer.scheduledTimer(withTimeInterval: 1, repeats: false) { [self] Timer in                             //1秒後にノートオフを送信する
            midiDevice?.noteOff(ch: 0, noteNumber: LMIDI.noteNumber(noteName: noteButton.title)!, velocity: 64)
        }
    }
